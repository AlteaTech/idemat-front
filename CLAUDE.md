...
**PR [#67](https://github.com/AlteaTech/idemat-front/pull/67) — feat(#440) : passages refusés, bloc IDemat `/consultation-solde` → `release/7.1-sprint15` (2026-08-27).** Nouveau bloc "Historique des passages refusés" entre "Historique des dépôts" et "Passages achetés", visible uniquement si `passagesRefuses().length > 0` (RG13.1 — pas de bloc vide, contrairement à "Historique des dépôts" qui affiche toujours un message même sans données). Réutilise telles quelles les classes `.histo-item`/`.btn-details`/`.btn-voir-tout`/`.points-debites` déjà en place — seule nouvelle classe locale : `.motif-refus-valeur` (même couleur que `.details-cell`, sans la grille 3 colonnes puisqu'il n'y a qu'une seule valeur à afficher). `PassagesRefusesIdematServiceAgents.getPassagesRefuses()` cast directement le DTO en modèle (même pattern que `getDepots()` — les noms de champs matchent 1:1, pas de mapper nécessaire). Dépend de #439 (back, [idbatv7#442](https://github.com/AlteaTech/idbatv7/pull/442)) pour tourner hors local. Redémarrage à froid du serveur fait par précaution avant/pendant cette PR (voir piège "ng serve qui ne recharge plus" ci-dessous — même famille de symptôme observée côté idbatv7-front sur la PR jumelle #438, cf. CLAUDE.md idbatv7-front).

## Statut PR en cours (vérifié 2026-07-22 tard)

**PR [#71](https://github.com/AlteaTech/idemat-front/pull/71) — fix(#70) : libellé "Nom du contact" → "Nom" en inscription professionnelle → `release/7.0` ✅ **mergée le 2026-09-09** (directement par nous, Jérémie ayant jugé le fix assez minimaliste pour ne pas nécessiter sa review — cf. mémoire `feedback_kanban_moves`/pratique habituelle où c'est lui qui merge).** Initialement ciblée `develop` (aucun sprint identifié au moment du fix), retargetée sur `release/7.0` (= sprint 14 côté idemat-front, nommage sans suffixe contrairement à `release/7.0-sprint14` sur idbatv7/idbatv7-front) après demande de Jérémie — branche recréée depuis `origin/release/7.0` (ancêtre strict de `develop`) via cherry-pick + `push --force-with-lease` + `gh pr edit --base`, PR/numéro conservés à l'identique. `inscription.component.html` — le `mat-label` du champ Nom affichait "Nom du contact" en mode Pro (incohérent avec "Prénom", jamais distingué). Retiré, toujours "Nom" quel que soit le type. ⚠️ **2 occurrences dans le même fichier** : `@if (isDesktop()) { ... } @else { ... }` (ligne 1) sont **deux templates entièrement séparés** (pas deux étapes d'un même formulaire) — desktop lignes ~1-249, mobile lignes ~249-fin. Tout futur fix sur ce formulaire (labels, validateurs, structure) doit être vérifié/dupliqué dans les deux blocs, un grep sur le texte concerné avant de considérer un fix "fini" sur cet écran.

**PR [#69](https://github.com/AlteaTech/idemat-front/pull/69) — fix(#452) : affichage matières des sites → `develop` ✅ mergée (2026-09-08).** Écran `/dechetterie/:id`, bloc "Matières acceptées" : RG1 `{{ matiere.tarif ?? 0 }} €` (un tarif `NULL` s'affichait vide au lieu de "0 €") ; RG2 ligne "Tarif" masquée par matière quand `masquerTarifs` (nouveau champ back, `DechetterieDetailDio`) est vrai — calculé côté back selon le type d'apporteur de l'usager + les flags Gratuit du contrat, voir CLAUDE.md idbatv7 pour le détail. Dépend de [idbatv7#456](https://github.com/AlteaTech/idbatv7/pull/456) (back). Vérifié en réel via l'API (curl + JWT, pas de compte de test navigable sous la main) — `masquerTarifs: true` confirmé pour un usager particulier sur un contrat gratuit ; le cas `masquerTarifs: false` (tarifs affichés normalement) reste à valider visuellement par Bertrand.
...
**PR [#53](https://github.com/AlteaTech/idemat-front/pull/53)** (#369, mergée 2026-08-07) — accord singulier/pluriel sur "passage(s) acheté(s)"/"passage(s) gratuit(s)" (`/consultation-solde`). **PR [#54](https://github.com/AlteaTech/idemat-front/pull/54)** (#368, mergée 2026-08-07) — voir pattern dédié ci-dessous (retour Payfip masque désormais la liste d'achat).

**PR [#56](https://github.com/AlteaTech/idemat-front/pull/56)** (#384, mergée 2026-08-12) — wording "Mes passages & points"→"Mes passages" (menu `idemat-shell.component.ts`) et titre `/consultation-solde` "Passages & Points"→"Passages" (`passages-points.component.html`). RG2 du ticket disait littéralement "Passagers" (coquille probable hors contexte métier) — corrigé en "Passages" après confirmation explicite de Bertrand.

**PR [#57](https://github.com/AlteaTech/idemat-front/pull/57)** (#383, mergée 2026-08-12) — affichage + contrôle des règles de complexité du mot de passe sur `/modification-mot-de-passe`, entre les 2 champs, même pattern que `reset-mot-de-passe` côté BO (`motDePasseComplexiteValidator`, 5 critères en direct). Remplace au passage `Validators.minLength(8)`, qui laissait passer des mots de passe rejetés ensuite par le back sans message clair. **PR [#58](https://github.com/AlteaTech/idemat-front/pull/58)** (#386, mergée 2026-08-12) — bloc "Informations" de `/consultation-solde` : retrait de "Nombre de passages achetés", wording "Forfait sur l'année" refondu, nouvelle ligne "Passages restants sur l'année" (N = forfait autorisé + forfait acheté − consommés). Dépend de [idbatv7#393](https://github.com/AlteaTech/idbatv7/pull/393) (#385) pour la vraie valeur du forfait autorisé (sinon affiche 0). **PR [#59](https://github.com/AlteaTech/idemat-front/pull/59)** (#388, mergée 2026-08-12) — voir pattern "centrage horizontal" ci-dessous, 9 écrans homogénéisés.

**PR [#62](https://github.com/AlteaTech/idemat-front/pull/62)** (#401, mergée 2026-08-13) — régression introduite par le #386/PR#58 : le séparateur `•` (bullet Unicode, en dur dans le texte du template, pas du CSS) entre les deux segments de la ligne "Forfait sur l'année" avait été remplacé par un `.` ASCII pendant la réécriture du wording. Un seul caractère corrigé, `passages-points.component.html:136`.

**PR [#63](https://github.com/AlteaTech/idemat-front/pull/63)** (#402, mergée 2026-08-13) — suppression de l'affichage `[Prénom] [Nom]` sous le titre (classe `.page-titre-sous`) sur 8 écrans de compte (`achat-passages`, `carte-acces`, `passages-points`, `informations-personnelles`, `modification-email`, `parametres-compte`, `modification-profil`, `modification-mot-de-passe`) — conservé uniquement sur `/home` (`.bienvenue-nom`) et dans le header desktop (`idemat-shell`). Règle CSS `.page-titre-sous` supprimée de `_common.scss` (devenue morte). Signal `usager` laissé tel quel dans les composants concernés (pas de nettoyage TS demandé, RG du ticket portait uniquement sur l'affichage).
...
## Piège — message d'erreur "mot de passe oublié" toujours faux (repéré 2026-08-06, non corrigé)

`mot-de-passe-oublie-idemat.component.html:38` affiche un texte fixe **"Il n'existe pas de compte avec cet e-mail"** pour **n'importe quelle** erreur renvoyée par le backend (`.ts` : `error: (err) => this.erreurEmail.set(true)`, sans distinguer la cause). Or côté back, `MotDePasseIdmService.demanderReset()` ne renvoie **jamais** d'erreur quand le compte n'existe pas — il répond `200 OK` silencieusement (anti-énumération volontaire, voir CLAUDE.md idbatv7). Conséquence : ce message ne peut littéralement jamais s'afficher pour la bonne raison — il ne sort QUE quand il y a une vraie panne technique (repéré via un vrai compte existant `rrosier@altea-si.com` tombant sur un SMTP local injoignable, voir CLAUDE.md idbatv7 pour la saga SMTP), et ment alors à l'usager en lui disant que son compte n'existe pas alors qu'il existe. **Pas encore corrigé** — proposition faite (remplacer par un message générique "Une erreur est survenue, merci de réessayer plus tard"), en attente de décision.
...
`.page-container` (global, `_common.scss`) se centre lui-même via `width: 50%; margin: 0 auto` (`width: 100%; margin: 0` sous 900px). Si un écran a besoin d'une largeur différente, poser le `max-width` **sur `.page-container` lui-même** (surcharge locale dans le composant), jamais sur un `div.contenu` enfant — sinon ce dernier ne se centre pas (il colle au bord gauche de la boîte large du parent), créant un désalignement horizontal entre écrans qui devraient pourtant être visuellement cohérents (repéré sur "Modifier le profil"/"Paramètres du compte" vs "Mon compte", 2026-07-22).

⚠️ **Convention tranchée (#388, 2026-08-12)** : ce piège touchait en réalité 9 écrans à la fois, pas un cas isolé — `achat-passages`, `modification-email`, `modification-mot-de-passe`, `mentions-legales` avaient un `max-width` sur `.contenu`/`.succes-bloc`/`.message-resultat` (enfant, jamais centré) ; `carte-acces`, `parametres-compte`, `modification-profil`, `informations-personnelles` avaient un `max-width: 720px` correctement posé sur `.page-container` mais **délibérément plus étroit** que le reste de l'app (harmonisation volontaire antérieure entre ces 4 écrans uniquement, commentaires explicites dans le code) ; `home` n'utilise même pas `.page-container` (structure `.home-container` dédiée) et plafonnait son contenu à `640px`. Décision Ronald après QA visuelle : **tous les écrans authentifiés doivent utiliser la largeur par défaut** (50%/100%), aucune exception — les 4 `max-width:720px` et le plafond `640px` de `home` ont été retirés, `home.component.scss` répliquant manuellement le comportement `width:50%/100%` de `.page-container` (structure différente, pas de refonte pour éviter le risque de régression). **Seul `/inscription` (`creation-compte`, écran public pré-connexion) garde son `max-width:720px` sur `.contenu`** — laissé de côté volontairement (hors périmètre de la demande, flux différent du reste de l'app authentifiée). Toute nouvelle exception de largeur doit désormais être justifiée explicitement, pas copiée d'un ancien écran.
...
⚠️ **Récidive (#388, 2026-08-12)** : le process `ng serve (idemat-front)` tournait depuis le **4 août** (8 jours), largement avant toute la session — Ronald a signalé "Mes accès n'a pas pris la largeur / Mentions légales non plus / Mon compte non plus" alors que le code corrigeant ces 3 écrans était bien correct sur disque. `ps aux` a confirmé un PID actif depuis `Mon09AM`/`4Aug26` — kill + `nohup npm start &` a immédiatement résolu les 3 écrans signalés. **Réflexe renforcé** : sur une session longue ou reprise après plusieurs jours, vérifier l'âge du process dev server (`ps aux | grep "ng serve"`) en préventif dès le premier changement de branche, pas seulement après qu'un fix semble "ne pas marcher" — ce piège s'est déjà reproduit plusieurs fois côté back (voir "Récidives" CLAUDE.md idbatv7) et touche maintenant aussi le front de façon récurrente.
...
## Piège — écran de retour Payfip doit reproduire le legacy dédié, pas superposer au formulaire (#368, résolu 2026-08-07)

`AchatPassagesComponent` réutilise le même écran/route pour le formulaire d'achat **et** le retour Payfip (query param `?resultat=P/A`, `resultatPaiement` signal) — contrairement au legacy .NET où `RetourPayfip.cshtml` est une page **entièrement séparée** de `AchatPassages.cshtml`, qui n'affiche jamais que l'en-tête + le message (`@Html.Raw(@Model.Variables.texteRetourPayfip)`), jamais la liste d'achat ni de bouton. Le composant v7 affichait le message de résultat (succès/échec) **en plus** de l'offre et du bouton "Valider" au lieu de les masquer — RG1 du ticket demandait explicitement de ne pas les afficher.

**Fix** : dans `achat-passages.component.html`, le `<p class="subtitle">` + `.contenu` (offre + bouton Valider) ne s'affichent que `@if (!resultatPaiement())` — sinon seul le message s'affiche, dans le même `@else if (options())` (les options restent chargées en tâche de fond, juste non rendues).

**Réflexe généralisable** : quand le legacy `.NET` a une page dédiée pour un état particulier (ici le retour d'un paiement), le fait qu'on regroupe cet état dans un composant Angular partagé (plus simple à router) ne dispense pas de reproduire le comportement "vue exclusive" — soit une route séparée, soit un `@if`/`@else` qui masque complètement l'autre état, jamais une simple superposition. Toujours vérifier l'archive `_ARCHIVE_DotNet_IDbat_Legacy/` pour la page correspondante avant d'improviser l'agencement d'un écran de confirmation/retour.
