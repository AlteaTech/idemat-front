...
{
    public class ParametresCompteModel : PageModel
    {
        private readonly IDemat.Data.IDematContext _context;
        private IWebHostEnvironment _environment;
        private readonly IConfiguration _config;

        public ParametresCompteModel(IDemat.Data.IDematContext context, IWebHostEnvironment environment, IConfiguration config)
        {
            _context = context;
        ...
        public async Task<IActionResult> OnGetAsync(string usagerGuid, bool supprime)
        {
            ...
            try
            {
                ...
                if (Variables.usageractif != null)
                {
                    ...
                    if (supprime)
                    {
                        Variables.urlContrat = _context.Contrats.Where(c => c.Id == Variables.usageractif.IdEnseigne).FirstOrDefault().URL;
                        ...
                        ListeNoire listenoire = new ListeNoire
                        {
                            GuidUsager = guidUsager,
                            IDMotifListeNoire = idMotif,
                            DateCreation = DateTime.Now.ToString("yyyyMMdd"),
                            HeureCreation = DateTime.Now.ToString("HHmmss")
                        };
                        _context.ListesNoires.Add(listenoire);
                        await _context.SaveChangesAsync();
                        
                        return RedirectToPage("./Index_svg", new { contrat = Variables.urlContrat });
                    }
                    else
                    {
                        return Page();
